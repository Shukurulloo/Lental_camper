...
const UsedComponents = () => {
  const [filteredData, setFilteredData] = useState(UsedCar);
  const [isBrandChecked, setIsBrandChecked] = useState (false);
  const handleClick = ()=> {
    setIsBrandChecked(!isBrandChecked);
    if (!isBrandChecked) {
      const filteredCar = [...UsedCar];
      const filteredCompany = filteredCar.filter((UsedCar) =>
        UsedCar.company.startsWith("다온티앤티")
        );
        setFilteredData(filteredCompany);
    }else{
      setFilteredData(...UsedCar);
    }
  }
      ...
      <Sidebar />
        <ItemBar>
          <Row>
            ...
            <Flex left>
            <div>Item: <span style={{color:'blue'}}>{UsedCar.length}</span></div>
            </Flex>
            <Flex right>
          ...
          <GridContainer>{UsedCar.slice(0, 24).map((item) => {
            return (
